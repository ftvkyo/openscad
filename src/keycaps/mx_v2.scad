...
module pts_slice(
    ...
    fn_z = max(24, ceil(abs(height) / 0.2));
...
module body(row) {
    pts_slice(
        ...
        scale_a = base_side / 2,
        scale_b = pad_side / 2,
        scale_ease = function(t) ease_out_asine(t)
    );
}


module body_depression(row) {
    translate([0, 0, pad_heights[row] + pad_depression / 2 + 0.01])
    pts_slice(
        height = -pad_depression,
        shape_a = f_pad,
        shape_b = f_circle,
        shape_ease = function(t) t,
        angle_a = pad_angles[row],
        angle_b = pad_angles[row],
        offset_a = pad_offsets[row],
        offset_b = pad_offsets[row],
        scale_a = pad_side / 2,
        scale_b = 1,
        scale_ease = function(t) ease_in_out_asine(t)
    );
}
